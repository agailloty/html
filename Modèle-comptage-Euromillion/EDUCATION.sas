PROC IMPORT OUT= WORK.EDUCATION                                                                                                           
            DATAFILE= "i:\SERVEUR ECONOMETRIE 2019-2020\
...
run;
proc print data=new2(obs=6);
var id choice nocoll cst2 cst3 niveau;
title 'Données empilées (6 obs)';
run;
...
b3=cst3*niveau;
run;
proc print data=new3(obs=6);
title 'interaction (6 obs)';
run;
...
proc print data=nelsout2(obs=6); 
